...
eval1 ((TmFun var params typ exp nextE),env,k) = return (nextE, functionAndParamEnv, k)
    where functionAndParamEnv = update (updateParams params env (createFuncParam var params 0)) var exp 
          updateParams :: [(String,NapType)] -> Environment -> [(String,Expr)] -> [[(String,Expr)]]
          updateParams [] env _= env
          updateParams (x:xs) env (y:ys) = updateParams xs (update env (fst y) (TmVar (fst x))) ys
